        ...
        <div class="page-title mb-4">
            <div class="row gy-3 align-items-center">
                <div class="col align-self-end">
                    <h1 class="h2 mb-0">Location List</h1>
                </div>
                ...
                <div class="col-auto">
                    <router-link :to="{name: 'add-location'}" class="btn rounded-pill btn-secondary-light">
                            ...
                            <table class="table align-middle mb-0 mw-lg">
                                <thead>
                                    <tr>
                                        ...
                                        <th width="20%" class="text-secondary">Image</th>
                                        <th width="30%" class="text-secondary fw-semibold">Location</th>
                                    ...
                                    <tr v-for="(obj, index) in locationList">
                                        ...
                                        <div class="form-check form-switch">
                                            <input
                                                ...
                                                role="switch"
                                                id="s1"
                                                :checked="obj.show_on_location_page == 1"
                                                @change="updateShowOnLocation(obj.id, $event)"
                                                true-value="1"
                                                false-value="0"
                                            >
                                        </div>
                                        ...
                                        <td>
                                            <ul class="action-btn-group mb-0 mw-0">
                                                <li>
                                                    <router-link :to="{name: 'add-location', params: { id: obj.id }}" class="btn ps-0 p-1 fs-5 text-black">
...
<script setup>
    ...
    const deleteItem = (v) => {
        ...
        function confirm(){
            axios.delete(`/api/location/${v}`).then(res => {
                if(res.data.status){
    ...
    const deleteMultiItem = () => {
        ...
        function confirm(){
            axios.post(`/api/location-delete-multiple`, {
    ...
    const updateStatus = (v, {target}) => {
        axios.post(`/api/location-update-status/${v}`, {
    ...
    const updateShowOnLocation = (v, {target}) => {
        axios.post(`/api/show-on-home-location/${v}`, {
            show_on_location: Number(target.checked)
        }).then(res => {
            if(res.data.status){
                toast(res.data.message, 'success').show()
            }
