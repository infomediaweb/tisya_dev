<template>
    <div class="page-wrap property-add">
        <div class="page-title mb-4">
            <div class="row gy-3 align-items-center">
                <div class="col align-self-end">
                    <h1 class="h2 mb-0">Collection List</h1>
                </div>
                ...
                <div class="col-auto">
                    <router-link :to="{name: 'add-collection'}" class="btn rounded-pill btn-secondary-light">
        ...
        <section class="section">  
            ...
            <template v-else>
                <div class="outer-wrapper" v-if="locationList.length">
                            ...
                            <table class="table align-middle mb-0 mw-lg">
                                <thead>
                                    <tr>
                                        ...
                                        <th width="30%" class="text-secondary">Image</th>
                                        <th width="30%" class="text-secondary fw-semibold">Collection</th>
                                        <th width="20%" class="fw-semibold">Show On Home</th>
                                        <th width="20%" class="text-secondary fw-semibold">Status</th>
                                        <th  style="width:90px;" class="text-secondary fw-semibold">Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-for="(obj, index) in locationList">
                                        <td>
                                            <input 
                                                class="form-check-input form-check-lg mt-0"
                                                ref="selectCheckRef"
                                        ...
                                        <td>
                                            <img
                                                :src="obj.image ? obj.image : placeholder"
                                                :alt="obj.collection_name"
                                                height="30"
                                            >
                                        </td>
                                        <td>{{ obj.collection_name }}</td>
                                        <td>
                                        <div class="form-check form-switch">
                                            <input
                                                ...
                                                role="switch"
                                                id="s1"
                                                :checked="obj.show_on_collection_page == 1"
                                                @change="updateShowOnCollection(obj.id, $event)"
                                                true-value="1"
                                                false-value="0"
                                            >
                                        </div>
                                        ...
                                        <td>
                                            <ul class="action-btn-group mb-0 mw-0">
                                                <li>
                                                    <router-link :to="{name: 'add-collection', params: { id: obj.id }}" class="btn ps-0 p-1 fs-5 text-black">
...
<script setup>
    ...
    const getLocationList = (pageNumber) => {
        ...
        axios.get('/api/collection', {
            ...
            if(res.data.status){
                locationList.value = res.data.data.data
    ...
    const deleteItem = (v) => {
        ...
        function confirm(){
            axios.delete(`/api/collection/${v}`).then(res => {
                if(res.data.status){
    ...
    const deleteMultiItem = () => {
        ...
        function confirm(){
            axios.post(`/api/collection-delete-multiple`, {
    ...
    const updateStatus = (v, {target}) => {
        axios.post(`/api/collection-update-status/${v}`, {
    ...
    const updateShowOnCollection = (v, {target}) => {
        axios.post(`/api/show-on-home-collection/${v}`, {
            show_on_collection: Number(target.checked)
        }).then(res => {
            if(res.data.status){
                toast(res.data.message, 'success').show()
            }
