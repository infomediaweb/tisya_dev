<template>
    <div class="page-wrap property-add">
        <div class="page-title mb-4">
            <div class="row gy-3 align-items-center">
                <div class="col align-self-end">
                    <h1 class="h2 mb-0">{{ route.params.id ? 'Modify Home Type' : 'Add Home Type' }}</h1>
                </div>
                <div class="col-auto">
                    <router-link :to="{name: 'list-home-type'}" class="btn rounded-pill btn-secondary-light">
                        <i class="icon-list me-2"></i>
                        Manage
                    </router-link>
                </div>
            </div>
        </div>


        <section class="section">
            <div class="d-flex justify-content-center py-5" v-if="isLoading">
                <div class="spinner-border" role="status"></div>
            </div>
            
            <div class="page-content" v-else>
                <Form action="" @submit="onSubmit" v-slot="{errors}">
                    <div class="row">
                        <div class="col-12 col-lg-10 col-xl-7">
                            <div class="row">
                                

                                <div class="col-12 col-lg-8">
                                    <div class="form-group">
                                        <label for="">Name<span class="text-danger">*</span></label>
                                        <Field 
                                            type="text" 
                                            name="name"
                                            :class="{'border-danger': errors.name}"
                                            class="form-control" 
                                            placeholder="" 
                                            v-model="rowEditData.name"
                                            rules="required"
                                        />
                                    </div>
                                </div>

                                <div class="col-12 mt-4">
                                    <div class="form-group">
                                        <h6 class="mb-0 fw-bold">SEO Fields</h6>
                                    </div>
                                </div>

                                <div class="col-12">
                                    <label>Title</label>
                                    <div class="form-group">
                                        <Field 
                                            type="text" 
                                            name="meta_title" 
                                            class="form-control"
                                            v-model="rowEditData.meta_title"
                                        />
                                    </div>
                                </div>

                                <div class="col-12 col-md-6">
                                    <label>Keywords</label>
                                    <div class="form-group">
                                        <Field 
                                            as="textarea" 
                                            name="meta_keyword" 
                                            class="form-control"
                                            rows="3"
                                            v-model="rowEditData.meta_keyword"
                                        />
                                    </div>
                                </div>

                                <div class="col-12 col-md-6">
                                    <label>Description</label>
                                    <div class="form-group">
                                        <Field 
                                            as="textarea" 
                                            name="meta_description" 
                                            class="form-control"
                                            rows="3"
                                            v-model="rowEditData.meta_description"
                                        />
                                    </div>
                                </div>

                                <div class="col-12">
                                    <div class="form-group">
                                        <div class="form-check form-switch">
                                            <input 
                                                name="status" 
                                                class="form-check-input"
                                                type="checkbox"
                                                v-model="vStatusChecked"
                                                true-value="1"
                                                false-value="0"
                                            >
                                        </div>
                                    </div>
                                </div>

                            </div>
                        </div>
                    </div>
                    <div class="row pt-2">
                        <div class="col-12">
                            <div class="form-group mb-0">
                                <button class="btn btn-save btn-primary">
                                    <div v-if="isSubmitLoading" class="spinner-border spinner-border-sm" role="status"></div>
                                    <span v-else>SUBMIT</span>
                                </button>
                            </div>
                        </div>
                    </div>
                </Form>
            </div> 
        </section>
        
    </div>
</template>


<script setup>
    import axios from 'axios'
    import { ref, onMounted } from 'vue'
    import { useRouter, useRoute } from 'vue-router'
    import { Form, Field, ErrorMessage } from 'vee-validate'
    import { toast } from '@utils/toast'

    const route = useRoute()
    const router = useRouter()
    const vStatusChecked = ref(1)
    const rowEditData = ref({})
    const submitApiUrl = ref(null)
    const isLoading = ref(false)
    const isSubmitLoading = ref(false)
    

    // For location edit 
    const dataEdit = () => {
        if(route.params.id){
            isLoading.value = true

            axios.get(`/api/hometype/${route.params.id}`).then(res => {
                if(res.data.status){
                    rowEditData.value = res.data.data
                    vStatusChecked.value = rowEditData.value.status

                    setTimeout(() => {
                        isLoading.value = false
                    }, 400)
                }
            }).catch(error => {
                isLoading.value = false
                console.log(error);
            })
        }
    }

    // For form on submit
    const onSubmit = (v, {resetForm}) => {
        isSubmitLoading.value = true

        if(route.params.id){
            submitApiUrl.value = `/api/hometype/${route.params.id}`
        }
        else{
            submitApiUrl.value = '/api/hometype'
        }
        
        axios.post(submitApiUrl.value, {
            name: v.name,
            meta_title: v.meta_title,
            meta_keyword: v.meta_keyword,
            meta_description: v.meta_description,
            status: vStatusChecked.value
        }).then(res => {
            if(res.data.status){
                toast(res.data.message, 'success').show()

                setTimeout(() => {
                    isSubmitLoading.value = false
                    router.push({name: 'list-home-type'})
                }, 400)
            }
        }).catch(error => {
            toast(error.response.data.message, 'error').show()
            isSubmitLoading.value = false
        })
 
    }

    onMounted(() => {
        dataEdit()
    })


</script>