<template>
    <div class="page-wrap property-add">
        <div class="page-title mb-4">
            <div class="row gy-3 align-items-center">
                <div class="col align-self-end">
                    <h1 class="h2 mb-0">Join Our Network - FAQs List</h1>
                </div>
                ...
                <div class="col-auto">
                    <router-link :to="{name: 'join-our-network-add-faq'}" class="btn rounded-pill btn-secondary-light">
        ...
        <section class="section">  
            ...
            <template v-else>
                <div class="outer-wrapper" v-if="faqsList.length">
                            ...
                            <table class="table align-middle mb-0 mw-lg">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-secondary fw-semibold">Question</th>
                                ...
                                <draggable 
                                    ...
                                    tag="tbody">
                                    <tr v-for="(obj, index) in faqsList" :key="index">
                                        <td>
                                            <input 
                                                class="form-check-input form-check-lg mt-0"
                                                ref="selectCheckRef"
                                        ...
                                        <td>
                                            <ul class="action-btn-group mb-0 mw-0">
                                                <li>
                                                    <router-link :to="{name: 'join-our-network-add-faq', params: { id: obj.id }}" class="btn ps-0 p-1 fs-5 text-black">
...
<script setup>
    ...
    // For get join our network all faqs list 
    const getList = (pageNumber) => {
        isLoading.value = true

        axios.get('/api/join-our-network-faqs', {
            params: {
                page: pageNumber,
    ...
    const deleteItem = (v) => {
        ...
        function confirm(){
            axios.delete(`/api/join-our-network-faqs/${v}`).then(res => {
                if(res.data.status){
    ...
    const deleteMultiItem = () => {
        ...
        function confirm(){
            axios.post(`/api/join-our-network-faqs-delete-multiple`, {
    ...
    const updateStatus = (v, {target}, idx) => {
        axios.post(`/api/join-our-network-faqs-update-status/${v}`, {
            status: Number(target.checked)
        }).then(res => {
            if(res.data.status){
                toast(res.data.message, 'success').show()
                faqsList.value[idx].status = Number(target.checked)
            }
        }).catch(error => {
            console.log(error)
        })
    ...
    // For remove active class and position change of faqs list
    const onDragChange = (e) => {
        let faqsListFilter = faqsList.value.map(item => item.id)

        axios.post('/api/join-our-network-faqs-save-position', {
            position: faqsListFilter
        }).then(res => {
            if(res.data.status){
                toast(res.data.message, 'success').show()
            }
