                                    ...
                                    <div class="row gx-3 gy-3">
                                        ...
                                        <div class="col">
                                           <!--  <img
                                                ...
                                                height="20"
                                            >&nbsp; -->
                                            <label :for="obj.id">{{ obj.tags_name }}</label>
                                        </div>
                                        
                                        <div class="col-12" v-if="false">
                                            <Field 
                                                type="text" 
                                                :name="`tags_number-${index}`"
                                                class="form-control"
                                                v-model="obj.number"
...
<script setup>
    ...
    const getAmenitiesList = () => {
        ...
        axios.get(`/api/get-home-tags/${route.params.id}`).then(res => {
            if(res.data.status){
                amenitiesList.value = res.data.data

                amenitiesList.value.filter(item => homeEdit.value.home_amenities.some(checkedItem => {
                    if(checkedItem.tags_id == item.id){
                        item.isChecked = true
                        item.number = checkedItem.tags_number
                    }
                }))
    ...
    const onSubmit = (v) => {
        ...
        let amenitiesListFilter = amenitiesList.value.filter(item => item.isChecked).map(item => ({
            ...
            tags_number: Number(item.number)
        }))

        axios.post('/api/save-home-tags', {
