       ...
       <div class="page-title mb-4">
           <div class="row gy-3 align-items-center">
               <div class="col align-self-end">
                   <h1 class="h2 mb-0">{{ route.params.id ? 'Join Our Network - Modify Faq' : 'Join Our Network - Add Faq' }}</h1>
               </div>
               <div class="col-auto">
                   <router-link :to="{ name: 'join-our-network-list-faqs' }" class="btn rounded-pill btn-secondary-light">
...
<script setup>
   ...
   const faqEdit = () => {
       if(route.params.id){
           ...
           axios.get(`/api/join-our-network-faqs/${route.params.id}`).then(res => {
               if(res.data.status){
   ...
   const onSubmit = (v) => {
       ...
       if(route.params.id){
           submitApiUrl.value = `/api/join-our-network-faqs/${route.params.id}`
       }
       else{
           submitApiUrl.value = '/api/join-our-network-faqs'
       }
       ...
       axios.post(submitApiUrl.value, v).then(res => {
           if(res.data.status){
               ...
               setTimeout(() => {
                   ...
                   router.push({name: 'join-our-network-list-faqs'})
               }, 400)
           }
       }).catch(error => {
