...
public function applyBookingCouponCode(Request $request){
  ...
  public function specialoffers(){
    ...
    function ajaxFilterProperties(Request $request){
        ...
        $list = $query->with(['images', 'locationData'])->where('status', 1)->whereNull('deleted_at')->get();
        ...
        foreach ($list as $property) {
            ...
            if($price > 0){
                ...
                if($price >= (integer)$request->minPrice && $count == 0){
                    $properties[] = $property;
                }
            }
        }
        ...
        $html = view('frontend.properties.ajax-filter-property', ['properties' => $paginator, 'nextPage'=>$nextPage, 'locationName'=>$locationName])->render();
