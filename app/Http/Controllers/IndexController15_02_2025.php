...
//             ->where('availability_date', '>=', date('Y-m-d'))
//             ->orderBy('availability_date', 'asc')
//             ->first();
    
//         $propertyUnavailableDates = DB::table('ru_property_availabilities')
//             ->where('ru_property_id', $property->ru_property_id)
...
//         //$adult = session('adultsCount', 1);
//       // $child = session('childrenCount', 0);
...
//                 // $next_date_after_checkout_date = date(
//                 //     "Y-m-d",
//                 //     strtotime($checkOutDate . "+1 days")
//                 // );
                
//                 // $count1 = DB::table("ru_property_availabilities")
//                 //     ->where("ru_property_id", $property->ru_property_id)
//                 //     ->where("availability_date", $last_date)
//                 //     ->where("is_available", "no")->count();
                
//                 // $count2 = DB::table("ru_property_availabilities")
//                 //     ->where("ru_property_id", $property->ru_property_id)
//                 //     ->where("availability_date", ">=",$checkOutDate)
//                 //     ->where("availability_date", "<=",$next_date_after_checkout_date)->where("is_available", "no")->count();
                
//                 // if($count1 == 0 && $count2 == 2){
//                 //     $checkOutDate = $last_date;
//                 // }    
...
//         return view('frontend.properties.property-detail', compact(
//             'property', 'no_of_nights', 'totGuest', 'adult', 'child', 
//             'checkInDate', 'checkOutDate', 'propertyUnavailableDates', 'previouslyBookedCheckoutDates',
 ...
 public function propertyDetail(Request $request, $home_type, $slug){
     ...
     $property = TblHome::with(['images','amenities','homeVideo','homeReviews','homeFeatures','tags'])
        ...
        //    $checkOutDate = '';
            ...
            return view('frontend.properties.property-detail', compact('property', 'no_of_nights', 'totGuest', 'adult', 'child', 'checkInDate', 'checkOutDate', 'propertyUnavailableDates', 'minStayArray','totalReviews', 'previouslyBookedCheckoutDates',
            'location_name', 'checkin_date', 
            'checkout_date', 'city_id', 
            'total_guests', 'adultsCount', 
            'childrenCount', 'guestCount', 'allowBooking'
            ));
}
...
public function applyBookingCouponCode(Request $request){
    ...
    public function PropertyList(Request $request, $slug=null){
        ...
        // $adultsCount = session('adultsCount');
    ...
    ->toArray(); 
        ...
        $totalStays = count($AllPropertyArray);
        return view('frontend.properties.property-list',compact('properties','sortOrder','locations','totalStays', 
  ...
  public function specialoffers(){
    ...
    //         $query = $query->with(['prices'=>function($query) use($request){
    //             $query->where('price', '>=', $request->minPrice)->where('price', '<=', $request->maxPrice)->where('price_date', '>=', date('Y-m-d'));
    //         }]);
    //     }
    //     if($request->has('tag') && $request->tag !='all'){
    //         $query = $query->with(['amenities'=>function($subquery) use($request){
    //             $subquery->where('amenities_id', $request->tag);
    //         }]);
    //     }
        
    //     $list = $query->with(['images'])->where('status', 1)->whereNull('deleted_at')->get();
    ...
    //         if(isset($propertyCheckInDate->availability_date)){
    //             if($request->has('minPrice') && $request->has('maxPrice')){
    //                 $price = RuPropertyPrice::where('ru_property_id', $property->ru_property_id)->where('price_date', $propertyCheckInDate->availability_date)->where('price', '>=', (integer)$request->minPrice)->where('price', '<=', (integer)$request->maxPrice)->sum('price');
    //             }
    //             else{
    //                 $price = RuPropertyPrice::where('ru_property_id', $property->ru_property_id)->where('price_date', $propertyCheckInDate->availability_date)->where('price_date', '>=', date('Y-m-d'))->sum('price');
    //             }
    //             if($price > 0){
    //                 if(setting()->website_markup){   
    ...
    //       $properties = collect($properties)->sortBy('pl_price')->values();
    //     }
    //     else{
           
    //       $properties = collect($properties)->sortByDesc('pl_price')->values();
    //     }
    //     $html = view('frontend.properties.ajax-filter-property', compact('properties'))->render();
        
    //     $location = TblLocation::where('id', $request->location)->first();
    //     if($location){
    //         $locartion= $location->location_name;
    //     }
    //     else{
    //       $locartion= 'All';
    //     }
